...
export default function Employees() {
  ...
  const fetchEmployees = async () => {
    try {
      const { data, error } = await tables.employees()
        .select('*')
        .order('name')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingEmployee) {
        // 수정
        const { error } = await tables.employees()
          .update(formData)
  ...
  const calculateSalary = (employee) => {
    ...
    const workDays = employee.work_days || 0
    const hoursPerDay = 8 // 기본 8시간 근무
    return hourlyWage * workDays * hoursPerDay
  }

  const totalSalary = employees.reduce((sum, employee) => sum + calculateSalary(employee), 0)
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
        ...
        <div className="card p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-gray-500">총 급여</p>
              <p className="text-2xl font-semibold text-gray-900">₩{totalSalary.toLocaleString()}</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700">근무일수 *</label>
                  <input
                    type="number"
                    required
